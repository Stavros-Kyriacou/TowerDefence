...
public class FireballTurret : Turret
{
    ...
    {
        //need a way to change target when it leaves the area
        //when doing check for disance < shortest distance, also check if currentTarget is in range of turret
        ...
        foreach (Collider2D enemy in enemiesInRange)
        {
            distanceFromCenter = Vector2.Distance(enemy.transform.position, Crystal.Instance.transform.position);
